...
void CubismState::init(ID3D11Device * device) {
    ...
    // モデルロード前に必ず呼び出す必要がある
    this->device = device;
    Live2D::Cubism::Framework::Rendering::CubismRenderer_D3D11::InitializeConstantSettings(2, device);
}


LAppModel::LAppModel(CubismState * cubismState) : CubismUserModel() , _modelSetting(NULL) , _userTimeSeconds(0.0f) , _deleteModel(false),cubismState(cubismState) {
...
void LAppModel::LoadAssets(const csmChar* dir, const csmChar* fileName) {
    ...
    csmSizeInt size;
    csmString d(dir);
    const csmString path = d + fileName;
    Buffer buffer = readFileToBufferA(path.GetRawString(),path.GetLength());
    ICubismModelSetting* setting = new CubismModelSettingJson((unsigned char*)buffer.getData(), buffer.getDataLength());
    SetupModel(setting);
    CreateRenderer();
    SetupTextures();
}
...
void LAppModel::SetupModel(ICubismModelSetting* setting)
{
    ...
    if (strcmp(_modelSetting->GetModelFileName(), "") != 0) {
        ...
        Buffer buffer = readFileToBufferA(path.GetRawString(),path.GetLength());
        LoadModel((unsigned char*)buffer.getData(), buffer.getDataLength());
    }
    ...
    if (_modelSetting->GetExpressionCount() > 0)
    {
        ...
        for (csmInt32 i = 0; i < count; i++)
        {
            ...
            path = _modelHomeDir + path;
            Buffer buffer = readFileToBufferA(path.GetRawString(),path.GetLength());
            ACubismMotion* motion = LoadExpression((unsigned char*)buffer.getData(), buffer.getDataLength(), name.GetRawString());
    ...
    if (strcmp(_modelSetting->GetPhysicsFileName(), "") != 0) {
        ...
        path = _modelHomeDir + path;
        Buffer buffer =readFileToBufferA(path.GetRawString(),path.GetLength());
        LoadPhysics((unsigned char*)buffer.getData(), buffer.getDataLength());
    }
    ...
    if (strcmp(_modelSetting->GetPoseFileName(), "") != 0) {
        ...
        path = _modelHomeDir + path;
        Buffer buffer = readFileToBufferA(path.GetRawString(),path.GetLength());
        LoadPose((unsigned char*)buffer.getData(), buffer.getDataLength());
    }
    ...
    if (strcmp(_modelSetting->GetUserDataFile(), "") != 0) {
        ...
        path = _modelHomeDir + path;
        Buffer buffer =readFileToBufferA(path.GetRawString(),path.GetLength());
        LoadUserData((unsigned char*)buffer.getData(), buffer.getDataLength());
    }
...
void LAppModel::PreloadMotionGroup(const csmChar* group) {
    ...
    for (csmInt32 i = 0; i < count; i++)
    {
        ...
        Buffer buffer = readFileToBufferA(path.GetRawString(),path.GetLength());
        CubismMotion* tmpMotion = static_cast<CubismMotion*>(LoadMotion((unsigned char*)buffer.getData(), buffer.getDataLength(), name.GetRawString()));
...
void LAppModel::Update(float delta)
{
    const csmFloat32 deltaTimeSeconds = delta;
...
CubismMotionQueueEntryHandle LAppModel::StartMotion(const csmChar* group, csmInt32 no, csmInt32 priority, ACubismMotion::FinishedMotionCallback onFinishedMotionHandler)
{
    ...
    if (motion == NULL)
    {
        ...
        Buffer buffer= readFileToBufferA(path.GetRawString(),path.GetLength());
        motion = static_cast<CubismMotion*>(LoadMotion((unsigned char*)buffer.getData(), buffer.getDataLength(), NULL, onFinishedMotionHandler));
...
void LAppModel::SetupTextures() {
    ...
    for (csmInt32 modelTextureNumber = 0; modelTextureNumber < _modelSetting->GetTextureCount(); modelTextureNumber++) {
        ...
        MultiByteToWideChar(CP_ACP,0,texturePath.GetRawString(),texturePath.GetLength(),tmp,1024);
        ComPtr<ID3D11ShaderResourceView> texture;
        ComPtr<ID3D11Resource> res;
        HRESULT hr = DirectX::CreateWICTextureFromFile(cubismState->getDevice(),tmp,res.GetAddressOf(),texture.GetAddressOf());
        //
        if (SUCCEEDED(hr)) {
            textures.push_back(texture);
                GetRenderer<Rendering::CubismRenderer_D3D11>()->BindTexture(modelTextureNumber, texture.Get());
            }
    }
    // premultであるなら設定
